...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['edit_machine'])) {
    ...
    // Handle Image Upload (Keep old if no new one provided)
    $image = $machine['image'];
    ...
    try {
        $stmt = $pdo->prepare("UPDATE machines SET slug=?, machine_name=?, description=?, capacity=?, video=?, image=?, features=?, specs=? WHERE id=?");
        $stmt->execute([$slug, $machine_name, $description, $capacity, $video, $image, $features_json, $specs_json, $id]);
        header("Location: index.php?msg=updated");
        exit;
    } catch (PDOException $e) {
        $error = "Error updating machine: " . $e->getMessage();
    }
}

// Prepare current data for display
$current_features = implode("\n", json_decode($machine['features'] ?? '[]', true));
$current_specs = json_decode($machine['specs'] ?? '{}', true);
?>
<!DOCTYPE html>
<html lang="en">
...
<head>
    <meta charset="UTF-8">
    <title>Edit Machine - Admin Panel</title>
            ...
            <form method="POST" action="" enctype="multipart/form-data">
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" name="video" class="form-control"
                            value="<?php echo htmlspecialchars($machine['video']); ?>">
                    </div>
                </div>

                <div class="mb-3">
                    <label class="form-label fw-semibold">Main Image (Leave blank to keep current)</label>
                    <?php if (!empty($machine['image'])): ?>
                        <div class="mb-2"><img src="<?php echo htmlspecialchars($machine['image']); ?>" width="100"
                ...
                <div class="mb-4">
                    ...
                    <textarea name="description" class="form-control" rows="4"
                        required><?php echo htmlspecialchars($machine['description']); ?></textarea>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <textarea name="features" class="form-control"
                        rows="5"><?php echo htmlspecialchars($current_features); ?></textarea>
                </div>
                    ...
                    <?php foreach ($current_specs as $key => $val): ?>
                        <div class="row g-2 mb-2 spec-row">
                            <div class="col-5"><input type="text" name="spec_key[]" class="form-control"
                                    value="<?php echo htmlspecialchars($key); ?>" placeholder="Property"></div>
                            <div class="col-6"><input type="text" name="spec_val[]" class="form-control"
                                    value="<?php echo htmlspecialchars($val); ?>" placeholder="Value"></div>
                ...
                <hr class="my-5">
                <button type="submit" name="edit_machine"
                    class="btn btn-primary btn-lg w-100 fw-bold rounded-pill shadow-sm">Update Machine</button>
            </form>
        </div>
    </div>
